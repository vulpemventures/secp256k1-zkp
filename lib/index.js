...
module.exports = () => {
  return new Promise((resolve) => {
    lib().then((Module) => {
       ...
       *  @arg {Uint8Array} point - point to compress
       *  @returns {Uint8Array} compressed point
       **/
      // eslint-disable-next-line no-unused-vars
      function pointCompress(point, _) {
        if (!point || !(point instanceof Uint8Array)) {
          throw new TypeError('point must be a Uint8Array');
        }
        const outputLen = 33;
        const outputLenPtr = malloc(8);
        Module.setValue(outputLenPtr, outputLen, 'i64');
        ...
        const ret = Module.ccall(
          ...
          'number',
          ['number', 'number', 'number', 'number'],
          [output, outputLenPtr, pointPtr, point.length]
        );
       ...
       */
      // eslint-disable-next-line no-unused-vars
      function pointFromScalar(scalar, _) {
        ...
        const ret = Module.ccall(
          ...
          'number',
          ['number', 'number', 'number'],
          [output, outputLenPtr, charStar(scalar)]
        );
        if (ret === 1) {
          const res = new Uint8Array(
            Module.HEAPU8.subarray(output, output + outputlen)
       ...
       * @return {Object} x-only point with tweak added and parity
       */
      function xOnlyPointAddTweak(point, tweak) {
        ...
        const parityPtr = malloc(4);
        const res = Module.ccall(
          'ec_x_only_pubkey_tweak_add',
          'number',
          ['number', 'number', 'number', 'number'],
          [output, parityPtr, pPtr, tweakPtr]
        );
        if (res === 0) {
          freeMalloc();
          throw new Error('x_only_pubkey_tweak_add', parity);
        }
        const pointTweaked = new Uint8Array(
          Module.HEAPU8.subarray(output, output + 32)
        );
        const parity = Module.getValue(parityPtr, 'i32');
        freeMalloc();
        return { xOnlyPubkey: pointTweaked, parity };
      }
      ...
      function verifySchnorr(message, publicKey, signature) {
        ...
        if (!publicKey || !(publicKey instanceof Uint8Array)) {
          throw new TypeError('public key must be a Uint8Array');
        }
