...
module.exports = () => {
  return new Promise((resolve) => {
    lib().then((Module) => {
      ...
      /**
       * @summary add a tweak to a point
       * @arg {Uint8Array} point - public key
       * @arg {Uint8Array} tweak - scalar to add to the point
       * @arg {boolean} compressed - whether the resulting point should be compressed
       * @returns {Uint8Array} public key resulting from the point addition
       */
      function pointAddScalar(point, tweak, compressed) {
        ...
        const lenghtPtr = malloc(8);
        const outputLen = compressed ? 33 : 65;
        Module.setValue(lenghtPtr, outputLen, 'i64');
