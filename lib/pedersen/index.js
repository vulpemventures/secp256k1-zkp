 ...
 *  @arg {number} value - unsigned 64-bit integer value to commit to.
 *  @exports
 */
 ...
 *  @throws {Error} - Decode error.
 *  @arg {Array} values - array of asset values.
 ...
 *  @exports
 */
function blindGeneratorBlindSum(values, nInputs, blindGenerators, blindFactors) {
  ...
  const lVal = values.map(v => Long.fromString(String(v), true).low);
  const hVal = values.map(v => Long.fromString(String(v), true).high);
  const ret = Module.ccall(
