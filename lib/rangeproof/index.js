...
function sign(
  ...
  if (!commit || !Buffer.isBuffer(commit) || !commit.length)
 ...
 *  @arg {Array} commit - 33-byte commitment.
 ...
 */
function verify(commit, proof, generator, extraCommit = []) {
 ...
 */
function rewind(commit, proof, nonce, generator, extraCommit = []) {
  if (!commit || !Buffer.isBuffer(commit) || !commit.length)
    throw new TypeError("commit must be a non empty Buffer");
  if (!proof || !Buffer.isBuffer(proof) || !proof.length)
    throw new TypeError("proof must be a non empty Buffer");
