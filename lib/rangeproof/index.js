 ...
 *  @arg {number} value: unblinded value.
...
function info(proof) {
  ...
  if (ret === 1) {
    const res = {
      ...
      minValue: Uint64Long(min).toNumber(),
      maxValue: Uint64Long(max).toNumber()
    };
    freeMalloc();
    return res;
  } else {
...
function rewind(commit, proof, nonce, extraCommit = []) {
  ...
  if (ret === 1) {
    ...
    const out = {
      value: Uint64Long(value).toNumber(),
      minValue: Uint64Long(minValue).toNumber(),
      maxValue: Uint64Long(maxValue).toNumber(),
